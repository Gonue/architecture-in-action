...
위와 같이 세션 저장소를 서버 내부 메모리를 사용하지 않고, RDBMS 나 Redis/Memcached와 같은 외부 공유 저장소 이다.

```
무상태(Stateless)
해당 시스템이 사용자의 이전 상태나 세션 정보를 저장하지 않는 것을 의미합니다. 
매 요청이 독립적으로 처리되며 이전 요청 또는 다음 요청에 영향을 주지 않습니다. 
예를 들어, HTTP 프로토콜은 기본적으로 무상태 프로토콜입니다. 
사용자가 웹 페이지를 요청할 때마다 서버는 각각의 요청을 독립적으로 처리하며 이전에 사용자가 
어떤 웹 페이지를 요청했는지 저장하거나 추적하지 않습니다.
```

```
상태유지(Stateful)
반대로 시스템이 사용자의 이전 상태나 세션 정보를 유지하거나 저장하는 것을 의미합니다. 
이는 연속된 요청 간에 정보를 전달하거나 특정 사용자의 세션 정보를 이용해 서비스를 제공하는 
등의 용도로 사용됩니다. 예를 들어, 로그인 세션을 유지하는 웹 서버는 사용자가 로그인한 상태를 
기억하고 있어야 하므로 상태유지의 특성을 가집니다.
```

따라서 무상태와 상태유지의 주요 차이점은 시스템이 사용자의 상태 정보를 저장하거나 유지하는지 여부에 있습니다.

### 데이터 센터

장애가 없는 상황에서 사용자는 가장 가까운 데이터 센터로 안내되는데, 통상 이 절차를 지리적 라우팅이라 부름.

![image](https://github.com/Gonue/architecture-in-action/assets/109960034/8b5a1fa7-db52-438e-a8f9-edf2583a5168)

만약 데이터 센터 중 하나에 심각한 장애가 발생하면 모든 트래픽은 장애가 없는 데이터 센터로 전송.

#### 다중 데이터 센터를 만들기 위해서 고려해야 할 조건
...
![image](https://github.com/Gonue/architecture-in-action/assets/109960034/0553bd3b-1664-44da-b104-077334cc051c)
...
![image](https://github.com/Gonue/architecture-in-action/assets/109960034/b0568bbc-7592-497d-a15d-716533d3e34c)
...
![image](https://github.com/Gonue/architecture-in-action/assets/109960034/253785a9-199a-4820-b831-55e975b337a5)
...
![image](https://github.com/Gonue/architecture-in-action/assets/109960034/9558eab4-e1f8-4e88-8aa4-884e6307a979)


#### 샤딩과 파티셔닝의 차이점
데이터베이스에서 샤딩(Sharding)과 파티셔닝(Partitioning)은 큰 데이터 세트를 관리하기 위한 두 가지 전략으로, 데이터를 여러 개의 더 작은 부분으로 나누는 방법입니다. 두 전략 모두 시스템의 가용성을 높이고 쿼리 성능을 향상시키는 데 도움이 됩니다.

파티셔닝
```
파티셔닝은 단일 데이터베이스 내에서 테이블을 논리적 또는 물리적으로 분할하는 프로세스입니다. 
파티셔닝은 특히 큰 테이블을 관리하는 데 도움이 됩니다. 예를 들어, 시간 기반 파티셔닝에서는 
테이블의 데이터를 시간 단위로 분할할 수 있습니다. 모든 데이터는 동일한 데이터베이스에 존재하지만, 
각 파티션은 별도로 쿼리되고 관리될 수 있습니다.
```

샤딩
```
샤딩은 데이터를 여러 데이터베이스에 분할하는 프로세스입니다. 
각 데이터베이스를 '샤드'라고 부르며, 각 샤드는 독립적인 데이터베이스 서버에서 호스팅될 수 있습니다. 
이렇게 하면 데이터베이스를 수평으로 확장할 수 있습니다(즉, 더 많은 서버를 추가하여 성능을 향상시킬 수 있습니다). 
샤딩은 분산 환경에서 데이터를 관리하는 데 특히 유용합니다.
```

따라서, 샤딩과 파티셔닝의 주요 차이점은 데이터가 어디에 위치하고 어떻게 분산되는지에 있습니다. 파티셔닝은 단일 데이터베이스 내에서 데이터를 분할하는 반면, 샤딩은 데이터를 여러 데이터베이스에 분산시킵니다.